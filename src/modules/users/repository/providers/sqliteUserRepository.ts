...
function findByIdSync(id: number): User | null {
  const db = getDb()
  const row = db
    .prepare(
...
export const sqliteUserRepository: UserRepository = {
  ...
  create(data: CreateUserDTO): User {
    ...
    return findByIdSync(result.lastInsertRowid as number)!
  },

  update(id: number, data: UpdateUserDTO): User | null {
    const db = getDb()
    const current = db.prepare('SELECT * FROM users WHERE id = ? AND deleted = 0').get(id) as UserRow | undefined
